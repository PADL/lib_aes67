...
#define GET_PTP_TRANSPORT_SPECIFIC(pkt)                                        \
    (((pkt)->transportSpecific_messageType & PTP_TRANSPORT_SPECIFIC_MASK) >> 4)

#define PTP_ANNOUNCE_TLV_TYPE (0x8)

// for PTPv2 over UDP
#define PTP_1588_DEST_ADDR                                                     \
    { 224, 0, 1, 129 }
#define PTP_1588_EVENT_PORT 319
#define PTP_1588_GENERAL_PORT 320

// for 802.1AS
#define PTP_8021AS_DEST_ADDR                                                   \
    { 0x01, 0x80, 0xc2, 0x0, 0x0, 0xe }
...
// Delay_Req has same structure as Sync message
typedef SyncMessage DelayReqMessage;
...
// PTP End-to-End delay response message
typedef struct {
    n80_t receiveTimestamp;
    n64_t requestingPortIdentity;
    n16_t requestingPortId;
} DelayRespMessage;
